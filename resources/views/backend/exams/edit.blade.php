...
@section('content')
    <form action="{{route('backend.exam.update',$exam->id)}}" method="POST" class="form-layout form-layout-2">
        {{csrf_field()}}
        {{method_field('PUT')}}
        <div >
            <div class="row no-gutters">
                <div class="col-md-8  mg-t--1 mg-md-t-0">
                    <div class="form-group ">
                        ...
                        <input class="form-control" type="text" name="title" placeholder="{{trans('admin.exam.title_placeholder')}}" value="{{$exam->title}}">
                    </div>
                </div><!-- col-8 -->
                <div class="col-md-4">
                    <div class="form-group mg-md-l--1">
                        ...
                        <select name="category_id" id="category" class="form-control" required>
                            ...
                            @foreach($categories as $category)
                                <option class="form-control" value="{{$category->id}}" @if($category->id == $exam->id) selected @endif>{{$category->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div><!-- col-4 -->
